...
function download_compose_file() {
    ...
    wget -q https://raw.githubusercontent.com/lww334455/p2p/main/docker-compose.yml -O docker-compose.yml
}
...
function set_image_version() {
    if [ -n "$version" ]; then
        export version
        echo -e "${green}Will use version: peer2profit/peer2profit_linux:$version ${plain}"
        sed -i "s/image:.*/image: peer2profit/peer2profit_linux:$version/g" docker-compose.yml
    else
        echo -e "${green}Will use defalut version: peer2profit/peer2profit_linux:latest ${plain}"
    fi
}
...
function set_proxy() {
    ...
    if [ ! -f ./proxychains4.conf ]; then
        ...
        wget -q https://raw.githubusercontent.com/lww334455/p2p/main/proxychains4.conf -O proxychains4.conf
    else
